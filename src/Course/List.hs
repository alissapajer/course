...
(++) ::
  ...
  -> List a
(++) Nil ys = ys
(++) xs ys = foldRight (\a bs -> a :. bs) ys xs
--(++) xs ys = foldLeft (\bs a -> a :. bs) ys xs
...
seqOptional (opt :. rest) = error "todo" --(bindOptional f opt)

--f :: a -> Optional (List a)
--f a = Full (a :. Nil)

--in bindOptional, b should be List b
...
find ::
  ...
  -> Optional a
find _ Nil = Empty
find f (a :. rest) = if (f a) then (Full a) else find f rest
...
reverse ::
  ...
  -> List a
reverse Nil = Nil
reverse (a :. rest) = (reverse rest) ++ a :. Nil
